...
7. **Risk Assessment → Controls step** — `applyControl` does not currently update `effectiveness_score` (stored as 0.0). Residual calculation uses this, so residual = inherent unless effectiveness is set elsewhere.
