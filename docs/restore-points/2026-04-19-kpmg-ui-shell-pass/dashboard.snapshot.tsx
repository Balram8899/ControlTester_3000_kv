    ...
    <div className="dashboard-panel card-interactive group flex flex-col overflow-hidden rounded-2xl border bg-card">
      ...
      <div className="flex flex-1 flex-col p-5">
        <div className="mb-3 flex items-start justify-between gap-2">
          <span className="text-sm font-semibold leading-tight text-[#0C233C]">{title}</span>
          {preview && (
        ...
        <p className="mb-2 text-xs text-muted-foreground">{subLabel}</p>
        <p className="mb-3 mt-auto text-[11px] text-muted-foreground">{detail}</p>
        <button
          type="button"
...
export default function DashboardPage() {
  ...
  return (
    <div className="flex h-full flex-col">
      <HeroSection
        title="Dashboard"
        subtitle="Audit overview - regulatory and controls intelligence"
      ...
      <div className="flex-1 overflow-auto p-6">
        <div className="mx-auto max-w-6xl space-y-6">
          ...
          <section>
            <div className="mb-4">
              <p className="kpmg-section-label">Analysis results</p>
              <h2 className="mt-1 text-lg font-bold">Explore diagnostic outputs</h2>
            </div>
            ...
            {analysisLoading ? (
              ...
              <>
                ...
                <div className="dashboard-highlight rounded-2xl p-6 text-white">
                  <div className="relative z-10">
                    <div className="mb-6 flex items-center justify-between gap-4">
                      ...
                      <button
                        ...
                        className="shrink-0 rounded-full border border-white/30 px-3 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-white/10 flex items-center gap-1.5"
                      >
                        View Full Analysis <ArrowRight className="h-3 w-3" />
                      </button>
                    </div>
                    ...
                    {frameworkNames.length > 0 && (
                      <div className="flex flex-wrap gap-2">
                        {frameworkNames.map((name, index) => (
                          <span
                            key={index}
                            className="rounded-full border border-white/20 bg-white/10 px-3 py-1 text-[11px] font-medium text-white/80"
                          >
                            {name}
                          </span>
                        ))}
